...
void anvil::TestingLayer::onEnter()
{
	...
	const auto& window = rle::Engine::get()->window();

	// Calculate the aspect ratio
	const float tx = window->width();
	const float ty = window->height();
	const auto aspect = tx / ty;
	...
	m_Camera = rle::factory<rle::Camera>::make_orthographic(-aspect, aspect, -1.0f, 1.0f);
...
void anvil::TestingLayer::onRender(rle::Renderer& renderer) const
{
	...
	for (const auto& shape : Shape::shapes)
		renderer.submit(shape.shader, shape.vao);
